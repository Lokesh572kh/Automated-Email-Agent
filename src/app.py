...
client = chromadb.PersistentClient(path="src/chroma_embedding")
...
def main():
    ...
    if submit_button:
        if user_input:
            st.session_state['conversation_history'].append(("Sender", user_input))
            output = process_message(user_input)
            st.session_state['conversation_history'].append(("Locks", output))
...
def process_message(message_content):
    ...
    try:
        if sender == "corporates":
            ...
            system_prompt2 = (
                "You are a helpful email assistant who helps in sending replies for the email based on the given scenarios and conditions. Just send the final reply only. Also your name is Locks."
                "Instructions for Replies (follow the most relevant, don't follow multiple instructions.): "
